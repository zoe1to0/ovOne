...
describe("Mobile MVP Product Shell", () => {
  ...
  it("shows a skippable handwritten text splash before entering chats", () => {
    ...
    assert.match(html, /\.mvp-splash \{[\s\S]*background: #faf6ec;/);
    ...
    assert.match(html, /\.mvp-splash-viewport \{[\s\S]*place-items: center;[\s\S]*padding: 0;[\s\S]*background: #faf6ec;/);
    assert.match(html, /\.mvp-splash-artwork \{[\s\S]*display: grid;[\s\S]*gap: 64px;[\s\S]*width: min\(62%, 258px\);[\s\S]*font-family: "Segoe Print", "Comic Sans MS", "Bradley Hand ITC", cursive;/);
    assert.match(html, /\.mvp-splash-logo \{[\s\S]*display: inline-flex;[\s\S]*gap: clamp\(12px, 4\.2vw, 24px\);/);
    assert.match(html, /\.mvp-splash-logo-o \{[\s\S]*font-size: clamp\(1\.95rem, 10\.2vw, 2\.95rem\);/);
    assert.match(html, /\.mvp-splash-logo-big-o \{[\s\S]*font-size: clamp\(2\.6rem, 12\.5vw, 3\.8rem\);/);
    assert.match(html, /\.mvp-splash-tagline \{[\s\S]*font-size: clamp\(0\.9rem, 3\.9vw, 1\.12rem\);/);
  ...
  it("renders chat pages with back, name, menu, scroll messages, fixed input, and message avatars", () => {
    ...
    assert.match(adapter, /return "输入消息…";/);
    assert.match(adapter, /hint\.textContent = "试用记忆：输入「记住：\.\.\.」"/);
    ...
    assert.match(html, /\.mvp-chat-view \{[\s\S]*background: #fbfaf3;/);
    assert.match(html, /\.mvp-chat-page-header \{[\s\S]*min-height: 58px;[\s\S]*border-bottom: 1px solid #e5edef;/);
    assert.match(html, /\.mvp-chat-menu-button \{[\s\S]*border: 1px solid #d4e7ec;[\s\S]*background: #edf8fa;/);
    assert.match(html, /\.mvp-message-stream \{[\s\S]*grid-auto-flow: row;[\s\S]*overflow-y: auto;[\s\S]*overscroll-behavior: contain;/);
    assert.match(html, /\.mvp-message \{[\s\S]*background: #fffdf7;[\s\S]*box-shadow: 0 4px 14px rgba\(84, 120, 130, 0\.06\);/);
    assert.match(html, /\.mvp-message-row\.is-mine \.mvp-message \{[\s\S]*background: #dff3f6;/);
    assert.match(html, /\.mvp-composer \{[\s\S]*position: fixed;[\s\S]*bottom: 64px;[\s\S]*border-top: 1px solid #e0eaec;/);
    assert.match(html, /\.mvp-avatar-wrap \.mvp-presence-dot \{[\s\S]*position: absolute;/);
    assert.match(html, /\.mvp-message-speaker \{/);
    assert.match(html, /\.mvp-message\.is-error \{[\s\S]*background: #fff8f5;/);
    assert.match(html, /\.mvp-message\.is-loading \{[\s\S]*background: #eef8f9;/);
    assert.match(html, /\.mvp-composer input::placeholder \{/);
    assert.match(html, /\.mvp-memory-hint \{[\s\S]*font-size: 0\.74rem;/);
    assert.match(html, /\.mvp-memory-notice \{[\s\S]*background: #e7f4ef;/);
