...
- Group files support metadata-only records on selected group chats and have pure real-upload/storage/retrieval/deletion/preflight contract boundaries, but no real upload, file binary/content storage, filesystem/cloud write, file parsing, retrieval/search, deletion execution, prompt injection, or AI runtime file reading exists yet.
