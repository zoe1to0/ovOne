...
- Me Settings Linked AI shows a disconnect confirmation scaffold from Global AI Links and has a deterministic cleanup plan scaffold, but confirmed global disconnect mutation is not implemented yet.
- Linked AI disconnect cleanup planning records affected worlds, contacts, private chats, memory scope ids, and unsupported group cleanup status; it does not execute Global AI Link/provider/world cleanup yet.
