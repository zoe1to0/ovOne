...
- Trial MVP scope is locked in `docs/trial-mvp-scope.md`, and local trial session entry plus the first Trial UI usability pass are implemented. Further visual polish can continue, but v0.17 intentionally did not change runtime/provider/memory architecture.
...
- Group files support metadata-only records on selected group chats and have pure real-upload/storage/retrieval/deletion/preflight contract boundaries. Deletion now has command validation and a descriptive plan, but no real upload, file binary/content storage, filesystem/cloud write, file parsing, retrieval/search, deletion execution, prompt injection, or AI runtime file reading exists yet.
