...
UI action
  ...
  -> FlowExecutor.run(action, { shell, state }) or FlowExecutor.runAsync(...) for SUBMIT_MESSAGE
...
- `resolveGroupAddMemberCandidates(...)` returns current-world AI contacts not already in the selected group, excluding ovO / the world system assistant.
...
- Group file deletion contract scaffolding exposes `GroupFileDeletionCommand`, `validateGroupFileDeletionCommand(...)`, `createGroupFileDeletionPlan(...)`, `canDeleteGroupFile(...)`, `canReadDeletedGroupFile(...)`, and `getGroupFileDeletionBoundary(...)`.
- `GroupFileDeletionPlan` is descriptive only: it does not delete file records, storage refs, group chat, messages/history, historical mentions, members, rules, appearance, contacts, private chats, memory scopes, or global/provider data.
